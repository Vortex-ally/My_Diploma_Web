...
def project_detail(request, project_id):
    ...
    if not (is_admin or is_owner or is_organiser):
...
def delete_project(request, project_id):
    ...
    if request.user == project.organiser or is_admin:
        Request.objects.filter(event=project).delete()
        project.delete()
        messages.success(request, "Проєкт видалено")
    else:
        messages.error(request, "У вас немає доступу для видалення цього проєкту")
    return redirect("dashboard")
